...
func (p *postgresServiceRepository) Create(ctx context.Context, s *domain.CreateServiceDto) (id uuid.UUID, err error) {
	query := `INSERT INTO "service" (name, url, token) VALUES ($1, $2, $3) RETURNING id;`
	err = p.Conn.QueryRowContext(ctx, query, s.Name, s.Name, "token").Scan(&id)
	if err != nil {
		return id, err
	}
	return id, nil
